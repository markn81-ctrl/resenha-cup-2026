...
const groups = {
  ...
  string,
  Array<{ code: string; countryCode: string; name: string; shortName: string; flagEmoji: string }>
>;
...
async function main() {
  ...
  for (const scope of [LeaderboardScope.OVERALL, LeaderboardScope.GROUP_STAGE, LeaderboardScope.KNOCKOUT]) {
    for (const [index, seed] of leaderboardSeeds.entries()) {
      const points =
        scope === LeaderboardScope.OVERALL ? seed.points : scope === LeaderboardScope.GROUP_STAGE ? seed.points - 22 : 22 + index * 2;
  ...
  console.log(`Seed concluido com ${createdTeams.length} selecoes e ${matches.length} jogos.`);
}
